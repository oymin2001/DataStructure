...
enum VertexStatus {UNEXPLORED, VISITED, NOT_FOUND};
enum EdgeStatus {DISCOVERY, BACK, CROSS, EDGE_UNEXPLORED, EDGE_VISITED,
                 EDGE_NOT_FOUND};
using namespace std;

class Graph //Based on Adjacency Matrix
{
public:
    class Vertex;
    class Edge;
...
public:
    ...
    public:
        ...
        Edge(Vertex v1,Vertex v2,int d)
            :vrtx_1(v1),vrtx_2(v2),distance(d),pVrtx_1(NULL),pVrtx_2(NULL),
        ...
        {
            if (v == *pVrtx_1) return *pVrtx_2;
            else if (v == *pVrtx_2) return *pVrtx_1;
            else
            {
                std::cout << "Error in opposite()" << '\n';
                return Vertex(NULL);
            }
        }
        Vertex getVertex_1() { return vrtx_1; }
        Vertex getVertex_2() { return vrtx_2; }
        Vertex* getpVertex_1() { return pVrtx_1; }
        Vertex* getpVertex_2() { return pVrtx_2; }
...
public:
    ...
    void insertEdge(Edge& e); // Edge e(vtx_1 -> vtx_2)
    void eraseEdge(Edge e);
    void eraseVertex(Vertex v);
    int getNumVertices() { return num_vertices; }
    void incidientEdges(Vertex v, EdgeList& edges);
    Vertex* getVrtxArray() { return pVrtxArray; }
    EdgeList* getAdjLstArray() { return pAdjLstArray; }
